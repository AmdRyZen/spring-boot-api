# -XX:+PrintGCDetails
#/Library/Java/JavaVirtualMachines/jdk-19.jdk/Contents/Home/bin/java -jar -XX:+UseZGC -XX:+UseNUMA -Xms8G -Xmx8G -XX:ReservedCodeCacheSize=128m -XX:InitialCodeCacheSize=128m -XX:ConcGCThreads=2 -XX:ParallelGCThreads=6 -XX:ZCollectionInterval=120 -XX:ZAllocationSpikeTolerance=5  ./target/spring-boot-api-0.0.1-SNAPSHOT.jar

sudo sh ../nacos/bin/shutdown.sh
sudo sh ../nacos/bin/startup.sh -m standalone


sh ../rocketmq-all-5.0.0-bin-release/bin/mqshutdown namesrv
sh ../rocketmq-all-5.0.0-bin-release/bin/mqshutdown broker

sleep 3

nohup sh ../rocketmq-all-5.0.0-bin-release/bin/mqnamesrv &
nohup sh ../rocketmq-all-5.0.0-bin-release/bin/mqbroker -n localhost:9876 autoCreateTopicEnable=true &

sleep 1
nohup /Library/Java/JavaVirtualMachines/jdk-19.jdk/Contents/Home/bin/java -jar ../zipkin/zipkin.jar &
